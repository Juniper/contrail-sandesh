...
class Sandesh(object):
    ...
    def set_logging_params(self, enable_local_log=False, category='',\
                           level=SandeshLevel.SYS_INFO, file=SandeshLogger._DEFAULT_LOG_FILE):
        self._sandesh_logger.set_logging_params(enable_local_log, category, level, file)
    ...
    def is_logging_allowed(self, sandesh_init):
        ...
        if logging_category == None or len(logging_category) == 0:
        ...
        try:
            ...
            if req_module:
                try:
                    ...
                    self._logger.error('Failed to import Module "%s"' % (req_module))
                else:
                    try:
                        sandesh_request = getattr(imp_module, request)()
                        return sandesh_request
                    except AttributeError:
                        self._logger.error('Failed to create Sandesh Request "%s"' % (request))
                        return None
            else:
                self._logger.error('Sandesh Request "%s" not implemented' % (request))
                return None
    ...
    def _process_rx_sandesh(self, rx_sandesh):
        ...
        if callable(handle_request_fn):
            ...
            self._logger.error('Sandesh Request "%s" not implemented' % (rx_sandesh.__class__.__name__))
        ...
        try:
            ...
            for importer, mod, ispkg in \
                ...
                if not ispkg:
                    ...
                    if 'ttypes' == module:
                        self._logger.debug('Add Sandesh requests in module "%s"' % (mod))
                        self._add_sandesh_request(mod)
                        self._logger.debug('Add Sandesh UVEs in module "%s"' % (mod))
    ...
    def _add_sandesh_request(self, mod):
        try:
            ...
            try:
                ...
                self._logger.error('"%s" module does not have sandesh request list' % (mod))
            else:
    ...
    def _add_sandesh_uve(self, mod):
        try:
            ...
            try:
                ...
                self._logger.error('"%s" module does not have sandesh UVE list' % (mod))
            else:
...
class SandeshUVE(Sandesh):
    ...
    def send(self, isseq=False, seqno=0, context='',
        ...
        if isseq is True:
            ...
            uve_type_map = sandesh._uve_type_maps.get_uve_type_map(self.__class__.__name__)
            if uve_type_map is None: return -1
            self._seqnum = self.next_seqnum()
