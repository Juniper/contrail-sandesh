...
#ifndef _SANDESH_TRANSPORT_TVIRTUALTRANSPORT_H_
#define _SANDESH_TRANSPORT_TVIRTUALTRANSPORT_H_ 1
...
class TTransportDefaults : public TTransport {
 public:
  ...
  int write(const uint8_t* buf, uint32_t len) {
    return this->TTransport::write_virt(buf, len);
  }
...
class TVirtualTransport : public Super_ {
 public:
  ...
  uint32_t readAll(uint8_t* buf, uint32_t len) {
    ...
    return ::contrail::sandesh::transport::readAll(*trans, buf, len);
  }
...
#endif // #ifndef _SANDESH_TRANSPORT_TVIRTUALTRANSPORT_H_
