...
#ifndef _SANDESH_TRANSPORT_TTRANSPORT_H_
#define _SANDESH_TRANSPORT_TTRANSPORT_H_ 1
...
int32_t readAll(Transport_ &trans, uint8_t* buf, uint32_t len) {
  ...
  while (have < len) {
    ...
    if (get <= 0) {
      LOG(ERROR, __func__ << ": No more data to read");
      return -1;
    }
    have += get;
  }
...
class TTransport {
 public:
   ...
   */
  virtual void open() {
    LOG(ERROR, __func__ << ": Cannot open base TTransport.");
    assert(false);
  }
   ...
   */
  virtual void close() {
    LOG(ERROR, __func__ << ": Cannot close base TTransport.");
    assert(false);
  }
   ...
   * @return How many bytes were actually read, -1 on error
   */
  ...
  virtual int32_t read_virt(uint8_t* /* buf */, uint32_t /* len */) {
    LOG(ERROR, __func__ << ": Base TTransport cannot read.");
    assert(false);
    return -1;
  }
   ...
   * @param len   How many bytes to read
   * @return How many bytes read, which must be equal to size, -1 on error
   */
   ...
   * discarded. Returns 0 on success, non-zero otherwise
   *
  ...
  int write(const uint8_t* buf, uint32_t len) {
    ...
    return write_virt(buf, len);
  }
  virtual int write_virt(const uint8_t* /* buf */, uint32_t /* len */) {
    LOG(ERROR, __func__ << ": Base TTransport cannot write.");
    assert(false);
    return -1;
  }
  ...
  virtual void consume_virt(uint32_t /* len */) {
    LOG(ERROR, __func__ << ": Base TTransport cannot consume.");
    assert(false);
  }
...
#endif // #ifndef _SANDESH_TRANSPORT_TTRANSPORT_H_
