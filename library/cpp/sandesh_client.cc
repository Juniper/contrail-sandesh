...
void SandeshClient::SendUVE(int count,
    ...
    map <string,vector<CategoryResultSubElem> > csevm;
    for (vector<SandeshMessageTypeStats>::const_iterator smit = mtype_stats.begin();
            smit != mtype_stats.end(); smit++) {
        vector<CategoryResultSubElem> csev;
        CategoryResultSubElem cse;
        cse.set_count(smit->get_stats().get_messages_sent());
        cse.set_category("ok");
        csev.push_back(cse);
        cse.set_count(smit->get_stats().get_messages_sent_dropped_no_queue());
        cse.set_category("no_queue");
        csev.push_back(cse);
        cse.set_count(smit->get_stats().get_messages_sent_dropped_no_client());
        cse.set_category("no_client");
        csev.push_back(cse);
        cse.set_count(smit->get_stats().get_messages_sent_dropped_no_session());
        cse.set_category("no_session");
        csev.push_back(cse);
        cse.set_count(smit->get_stats().get_messages_sent_dropped_queue_level());
        cse.set_category("queue_level");
        csev.push_back(cse);
        cse.set_count(smit->get_stats().get_messages_sent_dropped_client_send_failed());
        cse.set_category("client_send_failed");
        csev.push_back(cse);
        cse.set_count(smit->get_stats().get_messages_sent_dropped_session_not_connected());
        cse.set_category("session_not_connected");
        csev.push_back(cse);
        cse.set_count(smit->get_stats().get_messages_sent_dropped_header_write_failed());
        cse.set_category("header_write_failed");
        csev.push_back(cse);
        cse.set_count(smit->get_stats().get_messages_sent_dropped_write_failed());
        cse.set_category("write_failed");
        csev.push_back(cse);
        cse.set_count(smit->get_stats().get_messages_sent_dropped_wrong_client_sm_state());
        cse.set_category("wrong_client_sm_state");
        csev.push_back(cse);
        cse.set_count(smit->get_stats().get_messages_sent_dropped_validation_failed());
        cse.set_category("validation_failed");
        csev.push_back(cse);
        cse.set_count(smit->get_stats().get_messages_sent_dropped_rate_limited());
        cse.set_category("rate_limited");
        csev.push_back(cse);
        csevm.insert(make_pair(smit->get_message_type(), csev));
    }
    mcs.set_tx_msg_type(csevm);
