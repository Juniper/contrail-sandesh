...
class SandeshRequestTest : public ::testing::Test {
 protected:
    ...
    static SandeshSendingParamsSet* PrepareSendingParamsSetReq(
        uint32_t throttling_rate, bool enable_system_and_object_logs,
        bool disable_all_logs, int called_from_line) {
        SandeshSendingParamsSet *req(new SandeshSendingParamsSet);
        req->set_throttling_rate(throttling_rate);
        req->set_enable_system_and_object_logs(enable_system_and_object_logs);
        req->set_disable_all_logs(disable_all_logs);
        Sandesh::set_response_callback(
            boost::bind(ValidateSendingParamsResponse, _1, throttling_rate,
            enable_system_and_object_logs, disable_all_logs, called_from_line));
        return req;
    }
    ...
    static void ValidateSendingParamsResponse(Sandesh *response,
        ...
        EXPECT_EQ(throttling_rate, sresponse->get_throttling_rate());
        EXPECT_EQ(enable_system_and_object_logs,
            sresponse->get_enable_system_and_object_logs());
...
TEST_F(SandeshRequestTest, SendingParams) {
    // Get sending params
    uint32_t o_throttling_rate(Sandesh::get_send_rate_limit());
    bool o_enable_system_and_object_logs(
        !Sandesh::IsSendingSystemAndObjectLogsDisabled());
    bool o_disable_all_logs(Sandesh::IsSendingAllMessagesDisabled());
    // Set
    uint32_t throttling_rate(o_throttling_rate + 25);
    bool enable_system_and_object_logs(!o_enable_system_and_object_logs);
    bool disable_all_logs(!o_disable_all_logs);
    SandeshSendingParamsSet *req(PrepareSendingParamsSetReq(throttling_rate,
        enable_system_and_object_logs, disable_all_logs, __LINE__));
    validate_done_ = false;
    req->HandleRequest();
    req->Release();
    task_util::WaitForIdle();
    ...
    Sandesh::set_response_callback(boost::bind(ValidateSendingParamsResponse,
        _1, throttling_rate, enable_system_and_object_logs, disable_all_logs,
        __LINE__));
    validate_done_ = false;
    req1->HandleRequest();
    req1->Release();
    ...
    TASK_UTIL_EXPECT_EQ(true, validate_done_);
    // Set to original
    SandeshSendingParamsSet *req2(PrepareSendingParamsSetReq(o_throttling_rate,
        o_enable_system_and_object_logs, o_disable_all_logs, __LINE__));
    validate_done_ = false;
    req2->HandleRequest();
    req2->Release();
    task_util::WaitForIdle();
