...
public:
    ...
    virtual uint32_t ClearUVE(const std::string& proxy, int partition) = 0;
};
...
public:
    ...
    struct UVEMapEntry {
        UVEMapEntry(std::string table, std::string proxy, uint32_t seqnum):
                data(table, proxy), seqno(seqnum) {
        }
        U data;
        uint32_t seqno;
    };
    ...
    bool UpdateUVE(U& data, uint32_t seqnum, uint64_t mono_usec) {
        ...
        if (imapentry == a->second.end()) {
            std::auto_ptr<UVEMapEntry> ume(new
                    UVEMapEntry(data.table_, data.proxy_, seqnum));
...
public:
    static const int kPartitions = 30;
    ...
    uve_pmap * GetGMap(const std::string& proxy) {
        ...
        if (group_map_.find(proxy) == group_map_.end()) {
            uve_pmap up;
            for (size_t idx=0; idx<kPartitions; idx++) {
    ...
    bool UpdateUVE(U& data, uint32_t seqnum,
            uint64_t mono_usec, const std::string& proxy, int partition) {
        if (partition == -1) {
            assert(proxy.empty());
            return native_map_.UpdateUVE(data, seqnum, mono_usec);
        } else {
            assert(partition < kPartitions);
            uve_pmap * pp = GetGMap(proxy);
            return pp->at(partition).UpdateUVE(data, seqnum, mono_usec);
        }
    }

    // TODO:  Delete all UVE for the given partition for the given proxy group
    uint32_t ClearUVE(const std::string& proxy, int partition) {
        return 0;
    }
