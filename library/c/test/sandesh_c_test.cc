...
TEST_F(SandeshCTest, ReadWrite) {
    ...
    uuid_t uuid_test = {0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x09,0x0a,0x0b,0x0c,0x0d,0x0e,0x0f};
...
abc_sandesh_process (void *pabc) {
    u_int32_t i;
    uuid_t uuid_value = {0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x09,0x0a,0x0b,0x0c,0x0d,0x0e,0x0f};
...
buffer_test_process (void *ptr) {
    ...
    for (i = 0; i < 5; i++) {
        ...
        uuid_t uuid_temp = {0x00+i,0x00+i,0x01+i,0x01+i,0x02+i,0x02+i,0x03+i,0x03+i,0x04+i,0x04+i,0x05+i,0x05+i,0x06+i,0x06+i,0x07+i,0x07+i};
