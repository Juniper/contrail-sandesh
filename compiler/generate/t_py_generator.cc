...
void t_py_generator::generate_py_sandesh_definition(ofstream& out,
  ...
  */
  ...
  if (sandesh_type->is_sandesh_system() ||
    ...
    indent(out) << "threshold_buffer = collections.deque(maxlen=sandesh_base."
                   "Sandesh._DEFAULT_BUFFER_THRESHOLD)" << endl << endl;
    indent(out) << "buffer_overflow_logged = True" << endl << endl;
  }
