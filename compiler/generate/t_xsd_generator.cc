...
void t_xsd_generator::init_generator() {
  ...
  f_php_ <<
    "<?php" << endl;
#else
  // Make xsd output file
  string f_xsd_name = get_out_dir()+program_->get_name()+".xsd";
...
#ifdef SANDESH
void t_xsd_generator::generate_sandesh(t_sandesh* tsandesh) {
  vector<t_field*>::const_iterator m_iter;
  const vector<t_field*>& members = tsandesh->get_members();
  bool xsd_all = false;

  indent(s_xsd_types_) << "<xsd:complexType name=\"" << tsandesh->get_name() << "\">" << endl;
  indent_up();
  if (xsd_all) {
    indent(s_xsd_types_) << "<xsd:all>" << endl;
  } else {
