...
#ifdef SANDESH
  const std::vector<t_sandesh*>& get_sandeshs()  const  { return sandeshs_;  }
#endif
...
#ifdef SANDESH
  void add_sandesh  (t_sandesh*  ts) { sandeshs_.push_back(ts); }
#endif
...
#ifdef SANDESH
  t_struct* get_struct(const char *name) const {
    // Extract structs, exceptions, or unions matching the name
    std::vector<t_struct*> objects = get_objects();
    std::vector<t_struct*>::iterator o_iter;
    for (o_iter = objects.begin(); o_iter != objects.end(); ++o_iter) {
      if (strcmp(((*o_iter)->get_name()).c_str(), name) == 0) {
        return *o_iter;
      }
    }
    return NULL;
...
#ifdef SANDESH
  std::vector<t_sandesh*>  sandeshs_;
#endif
