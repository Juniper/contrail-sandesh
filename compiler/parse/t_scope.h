...
#ifdef SANDESH
  void add_sandesh(std::string name, t_sandesh* sandesh) {
    sandeshs_[name] = sandesh;
  }

  t_sandesh* get_sandesh(std::string name) {
    return sandeshs_[name];
  }
#endif
...
#ifdef SANDESH
  // Map of names to sandeshs
  std::map<std::string, t_sandesh*> sandeshs_;
#endif
